...
public class NormalLogin extends Activity implements
	...
	private class longOperation extends AsyncTask<String, Void, Long> {
			...
			try {
				...
				if (response.getStatusLine().getStatusCode() == 200) {
					...
					if (entity != null) {
						...
						// create a new json object and assign converted data as
						// params
						...
						if (username.equals(retUser)
								&& password.equals(retPass)) {
							// if(username.equals("pratik") &&
							// password.equals("qwerty")) {

							// create a new shared preference by getting the
							// preference
				...
				// Toast.makeText(getBaseContext(), "Connected failed!",
				// Toast.LENGTH_SHORT).show();
				result = 2;
			}
			return result;
		...
		protected void onPostExecute(Long result) {
			...
			case 1:
				Toast.makeText(getBaseContext(),
						"Invalid username and/or password", Toast.LENGTH_SHORT)
						.show();
				Main.isLogin = false;
				break;
			case 2:
	...
	/*
	 * @Override public void onBackPressed() { Main.isLogin = false; Session
	 * session = Session.getActiveSession(); if (!session.isClosed()) {
	 * session.closeAndClearTokenInformation(); } callMain();
	 * 
	 * }
	 */
